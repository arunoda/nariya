	...
	*/
	function deployNow(callback) {
		...
		if(workerPorts.length > 0) {
			...
			if(callback) callback({message: 'empty worker ports'});
		}
		...
		function startWorkers() {
			...
			workers.ready(function() {
				...
				if(errorStack.length == 0) {
					//no error
					eventBus.emit('notify.workersDeployed', workersToAdd);
					eventBus.emit('notify.deployed');
					if(callback) callback();
				} else {
					eventBus.emit('notify.error.workersDeploy', workersToAdd, err);
