	...
	this.deploy = function(appname, location, logpath, callback) {
		...
		function afterNpmInstalled(error, executed) {
			...
			if(!error) {
				...
				//starting the application (server.js)
				logger.info('starting the application with server.js');

				var outLogs = path.resolve(logpath, appname + '.log');
				var errLogs = path.resolve(logpath, appname + '.err.log');
				var appStartCommad = 'SL_NAME=' + appname + ' forever start -o ' + outLogs + ' -e ' + errLogs + ' / server.js';
				self.executeIfExists(location, 'server.js', appStartCommad, afterAppStarted);
			} else {
