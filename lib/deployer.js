	...
	this.deploy = function deploy(port, logFile, callback) {
		...
		function afterNpmInstalled(err, output) {
			...
			if(!err) {
				...
				//emit about the app
				logger.info('app started', {pid: app.pid});
				...
				app.on('exit', function (code) {
				  logger.error('child process exited with', {code: code, port: port});
				  eventBus.emit('appKilled', port, code);
				});

				//setting the master app Port
				masterApp = {
					port: port,
					path: appLocation,
					pid: app.pid
